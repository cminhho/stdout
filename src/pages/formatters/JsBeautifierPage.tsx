  ...
  return (
    <ToolLayout title={tool?.label ?? "JS Beautifier"} description={tool?.description ?? "Format and indent JavaScript code"}>
      <ToolOptions open={optionsOpen} onOpenChange={setOptionsOpen}>
        <input ref={fileInputRef} type="file" accept=".js,.mjs,.cjs,text/javascript" className="hidden" onChange={handleFileUpload} />
        <div className="flex flex-col gap-y-2.5 w-full">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
            <OptionField label="Upload your JS file">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="tool-panel">
          ...
          <CodeEditor value={output} readOnly language="javascript" placeholder={loading ? "Formatting…" : "Formatted code..."} />
        </div>
      </div>
    </ToolLayout>
  );
