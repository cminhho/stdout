# Tools Overview

stdout includes **62 tools** in one app. They are grouped by category in the sidebar and in the command palette.

---

## Formatters

| Tool | Description |
|------|-------------|
| JSON Format/Validate | Format, validate and beautify JSON with strict RFC compliance |
| JSON Repair | Repair relaxed JSON, JSONC, and JS-like object text into strict JSON |
| XML Format/Validate | Beautify, minify and validate XML |
| HTML Format/Validate | Beautify, minify and validate HTML |
| SQL Formatter | Format and beautify SQL queries |
| CSS Format/Minify | Beautify or minify CSS |
| JS Format/Minify | Beautify or minify JavaScript |
| CSS Inliner (Email) | Inline CSS styles into HTML for email templates |

---

## Encode and Crypto

| Tool | Description |
|------|-------------|
| Base64 Encode/Decode | Encode and decode Base64 strings |
| URL Encode/Decode | Encode and decode URL components |
| JWT Debugger | Decode and inspect JWT tokens |
| Hash Generator (MD5, SHA) | MD5, SHA-1, SHA-256 hashes |
| HTML Entity Encode/Decode | Encode and decode HTML entities |
| QR Code Generator | Generate QR codes from text or URLs |
| Gzip | Compress and decompress with Gzip |
| Certificate Inspector | Inspect and decode X.509 certificates |
| File Encoding Converter | Decode bytes from charset or encode text to UTF-8 |
| HMAC Generator | Generate HMAC signatures |

---

## Converters

| Tool | Description |
|------|-------------|
| Unix Timestamp Converter | Convert between Unix timestamps and dates |
| JSON ↔ YAML | Convert between JSON and YAML formats |
| CSV ↔ JSON | Convert between CSV and JSON formats |
| XML ↔ JSON | Convert between XML and JSON |
| Number Base Converter | Convert numbers between bases (bin, oct, dec, hex) |
| Color Converter | Convert CSS colors with alpha, contrast, and scales |
| Image ↔ Base64 | Convert images to/from Base64 strings |
| JSON → Table | Visualize JSON data as a table |
| JSON → Types | Generate TypeScript types from JSON |
| JSON ↔ Query String | Convert between JSON and URL query strings |
| .env Converter | Convert .env files to Netlify, Docker, YAML formats |
| CSS Units | Convert between CSS units (px, rem, em, vw) |
| XSD Generator | Generate minimal XSD schema from XML |
| XSLT Transformer | Transform XML using XSLT stylesheet |
| CSV → XML | Convert CSV to XML (first row as element names) |

---

## Validators

| Tool | Description |
|------|-------------|
| Regex Tester | Test regular expressions with live matching |
| JSONPath Tester | Test JSONPath expressions against JSON data |
| JSON Schema Validator | Validate JSON payloads against JSON Schema contracts |
| Credit Card Validator | Luhn check and generate test card numbers |
| XPath Tester | Run XPath expressions against XML |
| Schema Diff | Compare two JSON schemas side by side |
| Payload Comparator | Compare two JSON payloads and highlight differences |
| Cron Builder | Build and parse cron expressions (Quartz-style) |

---

## String and Utilities

| Tool | Description |
|------|-------------|
| Text Diff | Compare two texts and highlight differences |
| String Utilities | Case conversion, line ops (trim, sort, dedupe), escape quotes |
| String Escaper | Escape or unescape for JSON, XML, CSV, SQL, Java/.NET, JavaScript |
| Text Inspector | Character/byte/word/line counts, cursor position, word distribution |

---

## Generators

| Tool | Description |
|------|-------------|
| UUID Generator | Generate UUIDs (v1, v4, v7) |
| Password Generator | Generate secure passwords with custom rules |
| Random String Generator | Secure passwords, API tokens, secrets, OTPs, and readable codes |
| Mock Data Generator | Generate mock JSON data from a schema |
| ASCII Art Generator | Turn text into ASCII art |
| Log Generator | Generate synthetic log data for testing |

---

## Web Resources

| Tool | Description |
|------|-------------|
| Markdown Preview | Live preview of Markdown with GFM support |
| Lorem Ipsum | Generate placeholder text |
| URL Parser | Parse and inspect URL and query string |
| MIME Types Reference | Reference table of common MIME types |

---

## Networking and Other

| Tool | Description |
|------|-------------|
| cURL Builder | Build cURL commands visually |
| HAR Viewer | Inspect HAR (HTTP Archive) files |
| OpenAPI/Swagger Inspector | Validate, format, and extract endpoints from OpenAPI or Swagger specs |
| CSV Viewer | View and search CSV files in a table |
| Math Calculator | Evaluate math expressions |

---

## Image and Media

| Tool | Description |
|------|-------------|
| SVG Viewer | View, edit, and export SVG graphics |
| Image Resizer | Resize images with format conversion |

---

## Finding tools

- **Sidebar**: Tools are listed by group. Use the search box to filter by name or description.
- **Command palette**: Press **⌘K** (Mac) or **Ctrl+K** (Windows/Linux), then type the tool name.

See [Keyboard Shortcuts](keyboard-shortcuts.md) for more.
